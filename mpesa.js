// inclua a biblioteca
const Transaction = require('mpesa-mz-nodejs-lib')

// crie o objeto de configuração
const config = {
    public_key: '',
    api_host: 'api.sandbox.vm.co.mz',
    api_key: 'seenp11ug6n0a0pr50oa64ndltl8rpx5',
    origin: '*',
    service_provider_code: '171717',
    initiator_identifier: 'MyLunch',
    security_credential: '<Security Credential>'
}

// instancie o objeto Transaction, inicializando-o com a configuração válida
const transaction = new Transaction(config)
module.exports = transaction
// inicie uma transação C2B baseada em promessa
